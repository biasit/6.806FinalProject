...
class DataflowDataset(torch.utils.data.Dataset):
    ...
    max_src_length = MAX_SRC_LENGTH, max_trg_length = MAX_TRG_LENGTH,
    saved_indexes = SAVED_INDEXES):
        ...
        for v, id in self.saved_indexes.items():
            self.src_v2id[v] = id
            self.trg_v2id[v] = id

        # add in vocab in both vocabularies
        for v in sorted(list(both_src_trg_vocab - set(self.saved_indexes.keys()))):
...
# Retrieval Dataset (copy over with different name so can load pickle)
DataflowRetrievalDataset = type('DataflowRetrievalDataset',
  ...
  if not os.path.exists(dataset_path):
    if retrieval:
      dataset = DataflowRetrieval(data_src, src_vocab,
                                data_trg, trg_vocab,
                                MAX_SRC_LENGTH_RETR, MAX_TRG_LENGTH,
                                SAVED_INDEXES_RETR)
    else:
